package org.finlayfamily.littlefamily.imagefilters;

import android.graphics.Bitmap;

/**
 * Adapted to Android from Jerry's Java Image Processing library
...
public class LaplaceFilter {
    ...
    public void filter(Bitmap src, Bitmap dst) {
        ...
        getRGB( src, 0, 0, width, 1, row1 );
        getRGB( src, 0, 0, width, 1, row2 );
        brightness( row1 );
        brightness( row2 );
        for ( int y = 0; y < height; y++ ) {
            if ( y < height-1) {
                getRGB( src, 0, y+1, width, 1, row3 );
                brightness( row3 );
            }
        ...
        getRGB( dst, 0, 0, width, 1, row1 );
        getRGB( dst, 0, 0, width, 1, row2 );
        for ( int y = 0; y < height; y++ ) {
            if ( y < height-1) {
                getRGB( dst, 0, y+1, width, 1, row3 );
            }
            pixels[0] = pixels[width-1] = 0xff000000;//FIXME
            for ( int x = 1; x < width-1; x++ ) {
                int r = row2[x];
